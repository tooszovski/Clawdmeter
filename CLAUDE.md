...
- `boards/lilygo_tdisplay_s3_long/` — LilyGo T-Display-S3-Long (AXS15231B 180×640 QSPI LCD used in **landscape 640×180** via CPU-rotated strips; touch built into the AXS15231B @ 0x3B; PWM backlight GPIO1; SY6970 charger @ 0x6A for VBUS/charge; VBAT divider GPIO2). Build env: `lilygo_tdisplay_s3_long`. **One button** (BOOT/GPIO0 — the other key is RESET) in the PWR role: short = cycle, hold 3 s + release = pair, 8 s = deep sleep. Hold BOOT at power-on to flip the landscape orientation (NVS). Triggers the **wide two-column layout** in `ui.cpp` (`width >= 2*height`): one column per account from the payload's `accounts` array + "updated N ago" freshness line + a yellow/green **agent working/idle dot** under the battery, fed by the `ag` flag that `host/agent-status.js` (a Claude Code hook) → daemon produce (see `docs/statusline-source.md`). Sim twin: env `sim_long` (640×180, `sim/scenario_long.jsonl`).
